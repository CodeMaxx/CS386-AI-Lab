...
def dfsHelper(datum, x, y, visited):
    visited[(x,y)] = True

    if datum.getPixel(x, y) == 1:
        return

    if x != DIGIT_DATUM_WIDTH - 1 and not (x + 1, y) in visited:
        dfsHelper(datum, x + 1, y, visited)
    
    if y != DIGIT_DATUM_HEIGHT - 1 and not (x, y+1) in visited:
        dfsHelper(datum, x, y + 1, visited)
...
def enhancedFeatureExtractorDigit(datum):
    ...
    # Getting the vertical gradient
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(1, DIGIT_DATUM_HEIGHT):
            # If there is no change in the pixel value I mark it as 0 else 1
            if datum.getPixel(x, y) == datum.getPixel(x, y - 1):
                features[(x, y, "vgradient")] = 0
            else:
                features[(x, y, "vgradient")] = 1
    
    # Getting the horizontal gradient. similar to vertical gradient
    for y in range(DIGIT_DATUM_HEIGHT):
        for x in range(1, DIGIT_DATUM_WIDTH):
            if datum.getPixel(x, y) == datum.getPixel(x - 1, y):
                features[(x, y, "hgradient")] = 0
            else:
                features[(x, y, "hgradient")] = 1
    
    # Getting the vertical lines
    for x in range(DIGIT_DATUM_WIDTH):
        s = 0.0
        for y in range(DIGIT_DATUM_HEIGHT):
            s += datum.getPixel(x, y)
        val = s/DIGIT_DATUM_HEIGHT
        # If the percentage of pixels on the number in the image in a particular column
        # have more value than a certain threshold (which I tuned) then I take that there's
        # a vertical line in that column
        if val > 0.9:
            features[(x, "vlines")] = 1
        else:
            features[(x, "vlines")] = 0
    ...
    for y in range(DIGIT_DATUM_HEIGHT):
        ...
        for x in range(DIGIT_DATUM_WIDTH):
            s += datum.getPixel(x, y)
        val = s/DIGIT_DATUM_WIDTH
        # If the percentage of pixels on the number in the image in a particular row
        # have more value than a certain threshold (which I tuned) then I take that there's
        # a horizontal line in that row
        if val > 0.9:
            features[(y, "hlines")] = 1
        else:
            features[(y, "hlines")] = 0
    ...
    # and Checking if the number image is more in the left half or the right half
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            pixelval = datum.getPixel(x, y)
    ...
    # To mark the boundary of the image, I do DFS till I see a pixel on the number
    # Inside the boundary everyone is given 1 and outside given 0
    visited = {}
    dfsHelper(datum, 0, 0, visited)
