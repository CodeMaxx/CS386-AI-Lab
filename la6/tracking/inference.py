...
class ParticleFilter(InferenceModule):
    ...
    def __init__(self, ghostAgent, numParticles=300):
        ...
        # Added a variable to store the particles
        self.particles = []
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        particles = []
        num_legalpositions = len(self.legalPositions)
        
        # Get almost equal number of particles for all legal positions
        for i in range(self.numParticles):
            particles.append(self.legalPositions[i%num_legalpositions])
    ...
    def observe(self, observation, gameState):
        ...
        # If the particles is in jail
        if noisyDistance is None:
            # Change all the particles to jail position since the position is not fixed
            self.particles = [self.getJailPosition()]*self.numParticles
            return
        
        new_beliefs = util.Counter()
        # Stores sum of probability of all particles
        total_prob = 0
        new_particles = []
        old_beliefs = self.getBeliefDistribution()

        # Until we get atleast one non-zero probability particle
        while total_prob == 0:
            # Going over all particles
            # p is key of old_beliefs which is a particle
            for p in old_beliefs:
                # Getting true distance for the particle
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                # New belief for that particle = Prior*Likelihood
                new_beliefs[p] = old_beliefs[p]*emissionModel[trueDistance]
            total_prob = sum(new_beliefs.values())
            # If all the new particles have 0 probability
            # Get new uniformly distributed particles and start over
            if total_prob == 0:
                self.initializeUniformly(gameState)
                old_beliefs = self.getBeliefDistribution()
        ...
        # Sample new particles from the new belief distribution
        for i in range(self.numParticles):
            new_particles.append(util.sample(new_beliefs))
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        new_particles = []
        # Go through all the old particles
        for oldPos in self.particles:
            # Get the probability distribution of the new position
            # based on the old position of the ghost
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            # Sample a point from the new prob. distribution to get a new particle
    ...
    def getBeliefDistribution(self):
        ...
        # Getting the frequency of every particle in self.particles
        ...
        # Normalise it to get the probability distribution
        beliefs.normalize()
        return beliefs
        "*** YOUR CODE HERE ***"
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # A cartesion product of self.legalPosition in self.numGhost dimensions
        particles = list(itertools.product(*([self.legalPositions]*self.numGhosts)))
        # Shuffling particles
    ...
    def observeState(self, gameState):
        ...
        # Look for all ghosts in jail
        for i in range(len(noisyDistances)):
            ndist = noisyDistances[i]
            if ndist is None:
                jailed_ghosts.append(i)

        ## Updated particles if ghost goes in jail
        if len(jailed_ghosts) > 0:
            for i in range(len(self.particles)):
                p = list(self.particles[i])
                for j in jailed_ghosts:
                    p[j] = self.getJailPosition(j)
                self.particles[i] = tuple(p)

        new_beliefs = util.Counter()
        # Stores sum of probabilities of all particles
        total_prob = 0
        new_particles = []
        old_beliefs = self.getBeliefDistribution()
        ...
        while total_prob == 0:
            # For every old particle
            for w in old_beliefs:
                # For every ghost
                for g in range(self.numGhosts):
                    # If ghost is jailed skip
                    # Because the probability of being in jail will be 1
                    if g in jailed_ghosts:
                        continue
                    
                    # Get the true distance of this ghost according to this particle
                    trueDistance = util.manhattanDistance(w[g], pacmanPosition)
                    # Calculating Posterior = Prior * Likelihood
                    # Likelihood = Product of likelihoods of all ghosts independently
                    # Because all of the ghosts at a time are independent in the Bayes Net
                    new_beliefs[w] = new_beliefs[w]*emissionModels[g][trueDistance]
        ...
        # Get new particles by sampling the new distribution
        for i in range(self.numParticles):
            new_particles.append(util.sample(new_beliefs))
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            # For each ghost
            for g in range(self.numGhosts):
                # Get the new distribution based on the old position of ghosts
                # and how this ghost tends to move
                newPosDist = getPositionDistributionForGhost(
                    setGhostPositions(gameState, newParticle), g, self.ghostAgents[g])
                # Sample from this distribution to get a new position for this ghost
    ...
    def getBeliefDistribution(self):
        # Same as ParticleFilter Line 246
        beliefs = util.Counter()
        for i in self.particles:
            try:
                beliefs[i] += 1
