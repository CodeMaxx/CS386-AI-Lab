...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # If the ghost has been captured
        if noisyDistance is None:
            # Probability of all positions except jail position is 0
            for p in self.legalPositions:
                self.beliefs[p] = 0.0
            # Probability of jail position is 1.0
        ...
        for p in self.legalPositions:
            ...
            if emissionModel[trueDistance] > 0:
                # Probability of a position = Prior * Likelihood
                # True distance acts as a proxy for the position
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # For all legal positions of the ghost
        for p in self.legalPositions:
            # Get the distribution of new positions given the old position
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            for newPos, prob in newPosDist.items():
                # Prob(new position) = sum of probabs of reaching there from any of the old positions
                # self.beliefs[p] * prob =  P(oldPos) * P(newPost | oldPos)
